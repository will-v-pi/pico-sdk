...
// For configurable pins support, put an if () inside every #if

#if PICO_SUPPORT_CONFIGURABLE_PINS
#include "pico/configurable_pins.h"
#define STATUS_LED_USING_WL_GPIO (CYW43_WL_GPIO_LED_PIN >= 0)
#define STATUS_LED_USING_GPIO (PICO_DEFAULT_LED_PIN >= 0)
#define COLORED_STATUS_LED_USING_WS2812_PIO (PICO_DEFAULT_WS2812_PIN >= 0)
#else
...
static bool status_led_init_internal(__unused struct async_context *context) {
    ...
    if (COLORED_STATUS_LED_USING_WS2812_PIO) {
        ...
        if (PICO_DEFAULT_WS2812_POWER_PIN >= 0) {
